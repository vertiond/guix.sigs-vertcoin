...
fn test_check() {
    ...
    assert_eq!(
        check("B 22.1/user/all.SHA256SUMS").unwrap_err(),
        "File status for attestation is not 'A' (for add): 'B' '22.1/user/all.SHA256SUMS'"
    );
    assert_eq!(
        check("A 22.1/user/all.SHA256SUMS\nA 22.1/user/all.SHA256SUMS.ask").unwrap_err(),
        "Added unknown file '22.1/user/all.SHA256SUMS.ask'"
    );
    assert_eq!(
        check("A 22.1/user/all.SHA256SUMS").unwrap_err(),
        "Missing SHA256SUMS.asc or SHA256SUMS file in 22.1/user/all.SHA256SUMS"
    );
    assert_eq!(
        check("A 22.1/user/all.SHA256SUMS\nA 22.1/user/all.SHA256SUMS.asc"),
        Ok((vec!["22.1/user/all.SHA256SUMS"], HashSet::new()))
    );
    assert_eq!(
