...
fn test_check() {
    ...
    assert_eq!(
        check("B 0.18.0/user/all.SHA256SUMS").unwrap_err(),
        "File status is not 'A' (for add): 'B' '0.18.0/user/all.SHA256SUMS'"
    );
    assert_eq!(
        check("A 0.18.0/user/all.SHA256SUMS\nA 0.18.0/user/all.SHA256SUMS.ask").unwrap_err(),
        "Added unknown file '22.0/user/all.SHA256SUMS.ask'"
    );
    assert_eq!(
        check("A 0.18.0/user/all.SHA256SUMS").unwrap_err(),
        "Missing SHA256SUMS.asc or SHA256SUMS file in 0.18.0/user/all.SHA256SUMS"
    );
    assert_eq!(
        check("A 0.18.0/user/all.SHA256SUMS\nA 0.18.0/user/all.SHA256SUMS.asc"),
        Ok(())
    );
}
